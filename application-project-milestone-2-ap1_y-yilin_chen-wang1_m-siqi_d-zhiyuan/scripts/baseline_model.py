...
def perform_paired_t_tests(errors_dict):
    ...
    for i in range(len(models)-1):
        model_i_errors = errors_dict[models[3]]
        model_j_errors = errors_dict[models[i]]
        t_stat, p_value = stats.ttest_rel(model_i_errors, model_j_errors)
        p_values[f"{models[3]} vs {models[i]}"] = p_value
    return p_values
...
if __name__ == "__main__":
    ...
    print(out_of_sample_errors_neural_network)

    #_________feature_selection_NN__________
    # mean_ierr_nn = np.mean(in_sample_errors_neural_network)
    # mean_oerr_nn = np.mean(out_of_sample_errors_neural_network)
    # print("With all features:")
    # print(f"in sample error of Neural network: {mean_ierr_nn}")
    # print(f"out of sample error of Neural network: {mean_oerr_nn}")
    slc_ftrs = df[['acc_rate', 'track', 'm', 'n', 'current_pitch', 'current_roll', 'absoluate_roll', 'climb_delta', 'roll_rate_delta', 'climb_delta_diff', 'time1', 'omega', 'set']]
    slc_neural_network = model_evaluate(slc_ftrs, target, n_splits=10, model = MLPRegressor(hidden_layer_sizes=(100,), max_iter=1000, random_state=42))
    in_sample_errors_slc_nn, out_of_sample_errors_slc_nn = slc_neural_network.apply_k_fold('In-Sample vs. Out-of-Sample Errors for neural network', random_seed=42)
    print(out_of_sample_errors_slc_nn)
    # mean_ierr_snn = np.mean(in_sample_errors_slc_nn)
    # mean_oerr_snn = np.mean(out_of_sample_errors_slc_nn)
    # print("With selected features:")
    # print(f"in sample error of Neural network: {mean_ierr_snn}")
    # print(f"out of sample error of Neural network: {mean_oerr_snn}")
    ...
    'Hand-selected Features Model': out_of_sample_errors_slc_nn
    }
