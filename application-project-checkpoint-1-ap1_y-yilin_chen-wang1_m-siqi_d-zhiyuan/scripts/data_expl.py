...
def series_kde(series, names):
    plt.figure(figsize=(10, 6))
    sns.kdeplot(series, bw_adjust=0.5)
...
def multi_kde(df, ftr):
    ...
    for i, f in enumerate(ftr):
        i_row = i // n_cols
        i_col = i % n_cols
        sns.kdeplot(df[f], ax=axes[i_row, i_col], bw_adjust=0.5)
        axes[i_row, i_col].set_title(f'KDE of feature {f}')
        axes[i_row, i_col].set_xlabel(f'{f}')
